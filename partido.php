...
$sqlpartido = mysqli_query($conexionBD,"SELECT cod, TIME_FORMAT(hora, '%H:%i') AS hora, equipo_contrario, cod_categoria, DATE_FORMAT(fecha, '%d-%m-%Y') as fecha FROM partidos WHERE fecha >= CURDATE() ORDER BY `partidos`.`fecha` ASC;");
